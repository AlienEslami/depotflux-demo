...
JOBS_FILE = str(Path(tempfile.gettempdir()) / "depotflux-day-ahead-jobs.json")
    ...
    for solver_name in solver_candidates:
        try:
            ...
            if solver is None or not solver.available(False):
                continue
        # An unusable optional solver is expected and must not block fallback.
...
def solvePTO(sc, *, time_limit_seconds=None):
    ...
    if available_count == 0:
        raise RuntimeError("No supported MILP solver is available for day-ahead optimization")
