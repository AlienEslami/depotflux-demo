...
type SessionInfo = {
  subject: string;
  role: 'operator' | 'approver' | 'auditor' | 'admin';
  authentication_mode: 'disabled' | 'api_key';
};
...
export default function Home() {
  ...
  const [authMode, setAuthMode] = useState<'disabled' | 'api_key'>('disabled');
  ...
  const [knownApprovals, setKnownApprovals] = useState<
    Record<string, Approval | null>
  >({});
  const [notice, setNotice] = useState<OperationalNotice | null>(null);
  const [noticeScenario, setNoticeScenario] = useState<NoticeScenario>(
    'combined_disruption',
  );
  const [failureDrill, setFailureDrill] =
    useState<FailureDrillType>('infeasible');
  ...
  const [controlRejection, setControlRejection] =
    useState<ControlPolicyRejection | null>(null);
  ...
  const apiFetch = useCallback(
    (input: RequestInfo | URL, init: RequestInit = {}) => {
      ...
      if (accessKey.trim()) {
        headers.set('Authorization', `Bearer ${accessKey.trim()}`);
      }
    ...
    () =>
      runs.filter(
        (run) =>
          ...
          knownApprovals[run.id]?.decision === 'approved',
      ),
    [knownApprovals, runs],
  );
  ...
  const selectedControlRecord =
    selectedRunControlRecords.find(
      (record) =>
        record.interval_index === Number(controlInterval) &&
  ...
  const refreshWorkspace = useCallback(async () => {
    try {
      const [healthResponse, metaResponse] = await Promise.all([
        window.fetch(`${API_BASE}/health/ready`),
        window.fetch(`${API_BASE}/api/v1/meta`),
      ]);
      if (!healthResponse.ok || !metaResponse.ok) {
        throw new Error('The application service is not ready.');
      }
      setConnected(true);
      const capability = (await metaResponse.json()) as Capability;
      setAuthMode(capability.authentication_mode);

      const [inputResponse, runResponse, sessionResponse] = await Promise.all([
        apiFetch(`${API_BASE}/api/v1/inputs`),
        apiFetch(`${API_BASE}/api/v1/runs?limit=5&offset=0`),
        apiFetch(`${API_BASE}/api/v1/session`),
      ]);
      if (
        [inputResponse, runResponse, sessionResponse].some(
        ...
        setSessionInfo(null);
        setMessage(
          'Enter one of the local role access keys to open the workspace.',
        );
        return;
      }
      if (!inputResponse.ok || !runResponse.ok || !sessionResponse.ok) {
        throw new Error('The application service is not ready.');
      }
        ...
        runBody.items.map(async (run): Promise<[string, Approval | null]> => {
          ...
          try {
            const response = await apiFetch(
              `${API_BASE}/api/v1/runs/${run.id}/approval`,
            );
            return [
              run.id,
              response.ok ? ((await response.json()) as Approval) : null,
            ];
          } catch {
            return [run.id, null];
          }
      ...
      setSelectedReference(
        (current) => current || inputBody.items[0]?.reference || '',
      );
  ...
  const loadRun = useCallback(
    async (runId: string) => {
      try {
        const runResponse = await apiFetch(`${API_BASE}/api/v1/runs/${runId}`);
        ...
        const timelineResponse = await apiFetch(
          `${API_BASE}/api/v1/runs/${runId}/timeline`,
        );
        if (timelineResponse.ok) {
          const body = (await timelineResponse.json()) as {
            events: AuditEvent[];
          };
          setTimeline(body.events);
        }
        const noticeResponse = await apiFetch(
          `${API_BASE}/api/v1/runs/${runId}/notice`,
        );
        setNotice(
          noticeResponse.ok
        ...
        if (TERMINAL_STATUSES.has(run.status)) {
          const [resultResponse, approvalResponse] = await Promise.all([
            apiFetch(`${API_BASE}/api/v1/runs/${runId}/result`),
            apiFetch(`${API_BASE}/api/v1/runs/${runId}/approval`),
          ]);
          ...
          setRunResult(
            resultResponse.ok
              ? ((await resultResponse.json()) as RunResult)
              : null,
          );
          setApproval(loadedApproval);
          setKnownApprovals((current) => ({
            ...current,
            [runId]: loadedApproval,
          }));
        } else {
          setRunResult(null);
          setApproval(null);
        ...
        setMessage(
          error instanceof Error ? error.message : 'Run refresh failed.',
        );
      }
    },
    [apiFetch],
  ...
  const queueRun = useCallback(
    async (input: DemoInput, allowV2g: boolean) => {
      const response = await apiFetch(`${API_BASE}/api/v1/runs`, {
        method: 'POST',
        headers: {
  ...
  const queueFailureDrill = useCallback(
    async (input: DemoInput, drillType: FailureDrillType) => {
      const response = await apiFetch(`${API_BASE}/api/v1/failure-drills`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Idempotency-Key': crypto.randomUUID(),
      ...
      if (!response.ok) {
        const error = (await response.json()) as { message?: string };
        throw new Error(
          error.message ?? 'The failure drill could not be queued.',
        );
      }
      const created = (await response.json()) as Run;
      setRuns((current) => [
  ...
  const queueReplan = useCallback(
    async (baselineRunId: string, scenario: NoticeScenario) => {
      const response = await apiFetch(`${API_BASE}/api/v1/notices/simulate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Idempotency-Key': crypto.randomUUID(),
      ...
      if (!response.ok) {
        const error = (await response.json()) as { message?: string };
        throw new Error(
          error.message ?? 'The operational notice could not be submitted.',
        );
      }
      ...
      if (!runResponse.ok)
        throw new Error('The replanning run could not be loaded.');
      const candidate = (await runResponse.json()) as Run;
      setNotice(createdNotice);
      setRuns((current) => [
        candidate,
        ...current.filter((run) => run.id !== candidate.id),
      ]);
  ...
  async function submitDecision(decision: 'approved' | 'rejected') {
    ...
    try {
      const response = await apiFetch(
        `${API_BASE}/api/v1/runs/${selectedRunId}/approval`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      setMessage(
        `Candidate ${recorded.decision}; immutable audit record created.`,
      );
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : 'Failure drill submission failed.',
      );
    } finally {
      setDrilling(false);
    }
  ...
  async function cancelSelectedRun() {
    ...
    try {
      const response = await apiFetch(
        `${API_BASE}/api/v1/runs/${selectedRunId}/cancel`,
        {
          method: 'POST',
  ...
  async function simulateControlCommand(expectRejection: boolean) {
    ...
    try {
      ...
      if (expectRejection) {
          ...
          throw new Error(
            response.ok
              ...
              : (('message' in body ? body.message : undefined) ??
                  'The negative test failed unexpectedly.'),
          );
        }
        ...
        setMessage(
          `Policy correctly rejected interval ${intervalIndex}; no Modbus frame reached the controller.`,
        );
        return;
      }
      if (!response.ok) {
      ...
      const eventResponse = await apiFetch(
        `${API_BASE}/api/v1/security/events?limit=8`,
      );
      if (eventResponse.ok) {
      ...
      setMessage(
        error instanceof Error ? error.message : 'Control simulation failed.',
      );
    } finally {
      setDispatching(null);
    }
      ...
      context.registerTool(
        {
          ...
          description:
            'Queue a nominal day-ahead optimization for one registered demo input.',
          inputSchema: {
            type: 'object',
            properties: {
          ...
          async execute(value) {
            ...
            if (
              ...
              typeof input.v2g_enabled !== 'boolean'
            ) {
              throw new Error('input_reference and v2g_enabled are required.');
            }
            const demoInput = inputs.find(
              (item) => item.reference === input.input_reference,
            );
            if (!demoInput)
              throw new Error('The requested demo input is not registered.');
      ...
      context.registerTool(
        {
          ...
          description:
            'Queue a controlled infeasible-model or solver-timeout drill using a registered demo input.',
          inputSchema: {
            type: 'object',
            properties: {
              input_reference: { type: 'string' },
          ...
          async execute(value) {
            ...
            if (
              ...
              throw new Error(
                'A registered input_reference and supported drill_type are required.',
              );
            }
            const demoInput = inputs.find(
              (item) => item.reference === input.input_reference,
      ...
      context.registerTool(
        {
          ...
          description:
            'Cancel a queued run or request cancellation of a running optimization.',
          inputSchema: {
            type: 'object',
          ...
          async execute(value) {
            ...
            const response = await apiFetch(
              `${API_BASE}/api/v1/runs/${input.run_id}/cancel`,
              {
                method: 'POST',
                headers: { 'X-Operator-ID': 'demonstrator-operator' },
              },
            ...
            if (!response.ok) {
              const error = (await response.json()) as { message?: string };
              throw new Error(
                error.message ?? 'The run could not be cancelled.',
              );
            }
            const cancelled = (await response.json()) as Run;
            setRuns((current) => [
      ...
      context.registerTool(
        {
          ...
          description:
            'Preserve a frozen depot disruption and queue a remaining-horizon optimization from an approved baseline.',
          inputSchema: {
            type: 'object',
            properties: {
          ...
          async execute(value) {
            ...
            if (
              typeof input.baseline_run_id !== 'string' ||
              typeof input.scenario !== 'string' ||
              !scenarios.has(input.scenario)
            ) {
              throw new Error(
                'A baseline_run_id and supported scenario are required.',
              );
            }
    ...
    <main className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 flex h-16 items-center border-b border-white/8 bg-background/92 px-4 backdrop-blur-xl lg:px-7">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span
              className={`mr-1.5 size-1.5 rounded-full ${connected ? 'bg-emerald-300' : 'bg-amber-300'}`}
            />
            {connected === null
              ? 'Checking service'
              : connected
          ...
          <div className="hidden text-right sm:block">
            <p className="text-sm font-medium">
              {sessionInfo?.subject ?? 'Role session'}
            </p>
          ...
          <div className="grid size-9 place-items-center rounded-full bg-slate-700 text-sm font-semibold">
            {(sessionInfo?.role ?? 'local').slice(0, 2).toUpperCase()}
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto grid w-full max-w-[1600px] lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="hidden min-h-[calc(100vh-4rem)] border-r border-white/8 px-4 py-6 lg:block">
          ...
          <div className="mt-10 border-t border-white/8 pt-5">
            <p className="px-3 text-xs font-semibold uppercase tracking-[0.13em] text-muted-foreground">
              Operating boundary
            </p>
            <div className="mt-3 flex gap-3 rounded-xl border border-white/8 bg-white/[0.025] p-3">
              ...
              <p className="text-xs leading-5 text-slate-400">
                Human approval only. No direct control of buses or chargers.
              </p>
            </div>
          </div>
        </aside>
        ...
        <div className="grid min-w-0 gap-5 p-4 xl:grid-cols-[minmax(0,1fr)_340px] xl:p-7">
          <section className="min-w-0 space-y-5">
            {authMode === 'api_key' ? (
              <div className="flex flex-wrap items-center gap-3 rounded-xl border border-cyan-300/15 bg-cyan-300/[0.04] px-4 py-3">
                ...
                <div className="min-w-[180px] flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Session-only credential. Switch between operator, approver,
                    auditor, and administrator duties.
                  </p>
                </div>
                <input
                  id="role-access-key"
                  ...
                  className="h-9 w-full rounded-lg border border-white/10 bg-background/70 px-3 text-sm outline-none ring-cyan-300/40 placeholder:text-muted-foreground focus:ring-2 sm:w-72"
                />
                <Badge
                  variant="outline"
                  className="border-white/10 capitalize text-muted-foreground"
                >
                  {sessionInfo ? `${sessionInfo.role} active` : 'Locked'}
                </Badge>
              </div>
            ) : null}
            ...
            <Card className="border-white/10 bg-card/80 shadow-2xl shadow-black/15">
              ...
              <CardContent className="grid gap-5 pt-5 md:grid-cols-[minmax(0,1fr)_180px_auto] md:items-end">
                <div className="grid gap-2 text-sm font-medium">
                  ...
                  >
                    <SelectTrigger
                      id="fleet-data-set"
                      className="w-full border-white/10 bg-background/60"
                    >
                ...
                <div className="flex h-9 items-center justify-between rounded-lg border border-white/10 bg-background/60 px-3">
                  ...
                  <Switch
                    ...
                    aria-label="Enable vehicle-to-grid discharge"
                  />
                </div>
                <Button
              ...
              <div className="grid border-t border-white/8 sm:grid-cols-3">
                {[
                  {
                    label: 'Buses',
                    value: selectedInput?.fleet_size ?? '—',
                    Icon: BusFront,
                  },
                  {
                    label: 'Chargers',
                    value: selectedInput?.charger_count ?? '—',
                    Icon: BatteryCharging,
                  },
                  {
                    label: 'Service trips',
                    value: selectedInput?.trip_count ?? '—',
                    Icon: Waypoints,
                  },
                ].map(({ label, value, Icon }, index) => (
                  <div
                    key={label}
                    className={`flex items-center gap-3 px-5 py-4 ${index ? 'border-t border-white/8 sm:border-l sm:border-t-0' : ''}`}
                  >
            ...
            <Card className="border-rose-300/15 bg-card/80 shadow-2xl shadow-black/15">
              <CardHeader className="border-b border-white/8 pb-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <Badge
                    variant="outline"
                    className="border-rose-300/20 text-rose-200"
                  >
                    Never approvable
                  </Badge>
                </div>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="infeasible">
                        Infeasible · initial SOC below hard minimum
                      </SelectItem>
                      <SelectItem value="solver_timeout">
                        Timed out · 1 ms solver budget
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-xs font-normal leading-5 text-muted-foreground">
                    These frozen drills exercise the real mathematical model and
                    durable failure states. No result can reach operator
                    approval.
                  </p>
                </div>
                <Button
            ...
            <Card className="border-amber-300/15 bg-card/80 shadow-2xl shadow-black/15">
              ...
              <CardContent className="grid gap-5 pt-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
                <div className="grid gap-2 text-sm font-medium">
                  <label htmlFor="notice-scenario">
                    Frozen operational notice
                  </label>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="late_return">
                        Bus 1 · 30-minute late return
                      </SelectItem>
                      <SelectItem value="charger_derating">
                        Charger 1 · temporary 150 kW limit
                      </SelectItem>
                      <SelectItem value="combined_disruption">
                        Combined late return + charger derating
                      </SelectItem>
                      <SelectItem value="site_power_isolation">
                        Failure drill · all charging power unavailable
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-xs font-normal leading-5 text-muted-foreground">
                    The simulator preserves the source notice, applies
                    deterministic interpretation, and queues a real
              ...
              <CardContent className="grid gap-6 pt-5 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
                <div className="space-y-4">
                  <div className="grid gap-2 text-sm font-medium">
                    ...
                    <Select
                      value={
                        approvedRuns.some((run) => run.id === selectedRunId)
                          ? selectedRunId
                          : ''
                      }
                      onValueChange={(value) => {
                  ...
                  <div className="grid gap-2 text-sm font-medium">
                    ...
                    />
                    <p className="text-xs font-normal leading-5 text-muted-foreground">
                      Kept only in this page session. It is never stored in the
                      dashboard or audit record.
                    </p>
                  </div>
                  <div className="grid gap-2 sm:grid-cols-2">
                    <Button
                      disabled={
                        !controlEligible || !controlKey || dispatching !== null
                      }
                  ...
                  <p className="rounded-lg border border-white/8 bg-black/10 px-3 py-2.5 text-xs leading-5 text-slate-400">
                    The rejection test requests interval{' '}
                    {controlIntervals.length
                      ? controlIntervals.length + 1
                      : '—'}
                    , outside the optimized horizon. The API must refuse it
                    before encoding.
                  </p>
                </div>
                  ...
                  {selectedControlRejection ? (
                    <div className="rounded-lg border border-rose-300/20 bg-rose-300/[0.055] p-4">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <Badge
                          variant="outline"
                          className="border-rose-300/25 text-rose-200"
                        >
                          BLOCKED
                        </Badge>
                      </div>
                      <p className="mt-3 text-sm leading-6 text-slate-300">
                        {selectedControlRejection.reason_message}
                      </p>
                      <div className="mt-3 grid gap-2 font-mono text-xs text-rose-200/75 sm:grid-cols-2">
                        <span>
                          CHECK {selectedControlRejection.reason_code}
                        </span>
                        <span>
                          INTERVAL {selectedControlRejection.interval_index}
                        </span>
                      </div>
                      <p className="mt-2 break-all font-mono text-[11px] text-slate-500">
                        CORRELATION {selectedControlRejection.correlation_id}
                      </p>
                    </div>
                    ...
                    <div className="space-y-5">
                      <div className="flex flex-wrap items-end justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 font-mono text-3xl font-semibold text-cyan-200">
                            {formatNumber(
                              selectedControlRecord.setpoint_kw ?? undefined,
                              1,
                            )}{' '}
                      ...
                      <div>
                        ...
                        <p className="mt-2 text-xs text-muted-foreground">
                          Controller{' '}
                          {displayStatus(
                            selectedControlRecord.controller_response
                              ?.controller_state ?? 'unknown',
                          )}
                          {' · '}measured{' '}
                          {formatNumber(
                            selectedControlRecord.controller_response
                              ?.measured_site_power_kw,
                            1,
                          )}{' '}
                          kW
                          {' · '}heartbeat{' '}
                        ...
                        <p className="mt-2 break-all font-mono text-[11px] text-slate-500">
                          CORRELATION {selectedControlRecord.correlation_id}
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="grid min-h-72 place-items-center text-center">
                      <div className="max-w-sm">
                        ...
                        <p className="mt-3 text-sm font-medium">
                          No control evidence selected
                        </p>
                        <p className="mt-1 text-sm leading-6 text-muted-foreground">
                          Select an approved run, choose an interval, and
                          evaluate it against the OT policy gate.
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <Card className="border-white/10 bg-card/80">
                <CardHeader className="border-b border-white/8 pb-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">
                    Structured monitoring evidence
                  </p>
                  <CardTitle className="mt-1 text-lg">
                    Recent OT security events
                  </CardTitle>
                </CardHeader>
              ...
              <Card className="border-amber-300/15 bg-gradient-to-br from-amber-300/[0.055] to-card/80">
                <CardHeader className="border-b border-white/8 pb-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-amber-300">
                        Preserved source notice
                      </p>
                      <CardTitle className="mt-1 text-lg capitalize">
                        {displayStatus(notice.scenario)}
                      </CardTitle>
                    </div>
                    <Badge
                      variant="outline"
                ...
                <CardContent className="grid gap-5 pt-5 lg:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(notice.structured_facts.late_returns ?? []).map(
                        (item) => (
                          <Badge
                            key={`bus-${item.bus_id}`}
                            variant="outline"
                      ...
                      {(notice.structured_facts.charger_deratings ?? []).map(
                        (item) => (
                          <Badge
                            key={`charger-${item.charger_id}`}
                            variant="outline"
                            className="border-white/10 text-slate-300"
                          >
                            Charger {item.charger_id} {item.from_kw}→
                            {item.to_kw} kW
                          </Badge>
                        ),
                      )}
              ...
              <Card className="border-cyan-300/15 bg-card/80">
                <CardHeader className="border-b border-white/8 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">
                        Candidate versus approved baseline
                      </p>
                      <CardTitle className="mt-1 text-lg">
                        {runResult.result.comparison.changed_intervals}{' '}
                        intervals revised
                      </CardTitle>
                    </div>
                  </div>
                ...
                <CardContent className="grid gap-px overflow-hidden p-0 sm:grid-cols-4">
                  {[
                    [
                      'Remaining cost',
                      runResult.result.comparison.delta.remaining_cost,
                      '$',
                    ],
                    [
                      'Grid purchase',
                      runResult.result.comparison.delta.grid_purchase_kwh,
                      ' kWh',
                    ],
                    [
                      'V2G export',
                      runResult.result.comparison.delta.v2g_export_kwh,
                      ' kWh',
                    ],
                    [
                      'Peak import',
                      runResult.result.comparison.delta.peak_import_kwh,
                      ' kWh',
                    ],
                    ...
                    return (
                      <div
                        key={label as string}
                        className={`p-5 ${index ? 'border-t border-white/8 sm:border-l sm:border-t-0' : ''}`}
                      >
                        <p className="text-xs text-muted-foreground">
                          {label as string} delta
                        </p>
                        <p
                          className={`mt-2 font-mono text-lg font-semibold ${numericValue > 0 ? 'text-amber-200' : numericValue < 0 ? 'text-emerald-300' : 'text-slate-200'}`}
                        >
                          {unit === '$'
                            ? `${numericValue >= 0 ? '+' : ''}$${formatNumber(numericValue, 2)}`
                            : `${numericValue >= 0 ? '+' : ''}${formatNumber(numericValue, 1)}${unit as string}`}
                        </p>
                      </div>
                    );
                  })}
                  ...
                  <div className="grid min-h-44 place-items-center px-6 py-10 text-center">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        Connect the service and queue the first frozen scenario.
                      </p>
                    </div>
                  </div>
                )}
                      ...
                      >
                        <div className="relative flex justify-center">
                          ...
                          {index < timeline.length - 1 ? (
                            <span className="absolute top-4 h-[calc(100%+0.7rem)] w-px bg-white/10" />
                          ) : null}
                        </div>
                        <div>
                          <p className="text-sm font-medium capitalize">
              ...
              <CardContent>
                <div className="relative mb-5 h-44 overflow-hidden rounded-xl border border-white/8 bg-[#07101a] p-2">
                  {chartData.length ? (
                    ...
                    >
                      ...
                      <path
                        d="M0 105 C36 102 45 90 78 93 S128 44 162 54 208 90 240 59 276 37 300 41"
                        fill="none"
                ...
                {runResult?.result ? (
                  <div className="mb-5 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-white/8 bg-white/8">
                    {[
                      [
                        selectedRun?.run_type === 'real_time'
                          ? 'Remaining cost'
                          : 'PTO cost',
                        `$${formatNumber(runResult.result.pto_daily_cost, 2)}`,
                      ],
                      [
                        'Aggregator value',
                        `$${formatNumber(runResult.result.aggregator_revenue, 2)}`,
                      ],
                      [
                        'Grid purchase',
                        `${formatNumber(runResult.result.total_kwh_bought)} kWh`,
                      ],
                      [
                        'V2G export',
                        `${formatNumber(runResult.result.total_kwh_sold)} kWh`,
                      ],
                    ].map(([label, value]) => (
                ...
                {runResult?.failure_message ? (
                  <div className="mt-5 rounded-lg border border-rose-400/20 bg-rose-400/5 p-3 text-sm text-rose-200">
                    ...
                    {runResult.failure_code ? (
                      <p className="mt-1 font-mono text-xs text-rose-300/70">
                        {runResult.failure_code}
                      </p>
                    ) : null}
                  </div>
                ...
                {selectedRun?.recovery_count ? (
                  <p className="mt-5 rounded-lg border border-blue-400/20 bg-blue-400/5 p-3 text-sm text-blue-200">
                    Worker recovery completed {selectedRun.recovery_count} time
                    {selectedRun.recovery_count === 1 ? '' : 's'}; the run
                    remained durable.
                  </p>
                ) : null}
                {selectedRun &&
                ['queued', 'running', 'cancel_requested'].includes(
                  ...
                  <Button
                    variant="outline"
                    disabled={
                      cancelling || selectedRun.status === 'cancel_requested'
                    }
            ...
            {approval ? (
              ...
              >
                <CardContent className="pt-5">
                  <div className="flex items-start gap-3">
                    <CheckCircle2
                      className={`mt-0.5 size-5 ${approval.decision === 'approved' ? 'text-emerald-300' : 'text-rose-300'}`}
                    />
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {approval.note || 'No operator note supplied.'}
                      </p>
              ...
              <CardContent className="flex gap-3 pt-5">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-400">
                    Approval creates an audit record. It never dispatches
                    commands to physical assets.
                  </p>
                </div>
              </CardContent>
            </Card>
      ...
      <Dialog open={decisionOpen} onOpenChange={setDecisionOpen}>
        <DialogContent className="border-white/10 bg-[#0d1a26] sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              This decision is immutable and bound to result hash{' '}
              {runResult?.result_sha256?.slice(0, 12) ?? '—'}. It does not
              dispatch a charger command.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-2 text-sm font-medium">
